 ...
 *=============================================================
 * ORDER PDF VIEW
 * This part well apply when you go to view orders PDF
 * EX: [WewSite]?module=orders&action=view&id=[1]
 *=============================================================
 */
	session_start();
	header('Content-Type: text/html; charset=UTF-8');

	//we have id_quotation in url
	if ( empty($_GET['id_quotation']) ) return;

	//quotation
	$id_quotation = intval($_GET['id_quotation']);
	if ( intval($_GET['id_quotation']) <= 0 ) return;
	...
	//payment_choice
	$payment_choice = $q['quotation']['payment_choice'];
	if( $payment_choice != "" ){
		$methods = $common->select('payment_methodes', array('value'), "WHERE id IN($payment_choice)" );
	}
	if( $q['quotation']['id_payment_method'] != "0" ){
		$payment_method = $common->select('payment_methodes', array('value'), "WHERE id=".$q['quotation']['id_payment_method'] );
	}
	//var_dump($payment_method);


	//currency
	$currency = $q['quotation']['currency_sign'];
	...
	//total
	$total_products  = $q['total']['product_tht'];
	$total_ht 			 = $q['total']['tht'];
	$global_weight   = $q['total']['weight'];
	$global_discount = $q['quotation']['global_discount']; 
	//quotation infos
	$voucher_code 	 = $q['quotation']['voucher_code'];
	$voucher_value 	 = $q['quotation']['voucher_value']; 
	$avoir 					 = $q['quotation']['avoir'];
	//carrier infos
	$shipping_costs  = number_format($q['carrier']['shipping_costs'], 2);
	...
	$html .= '<div style="width:1024px;margin:0px auto;">';
		...
		$html .= '<table border="0" cellpadding="5" cellspacing="0" style="width:1024px;">';
			...
			$html .= '<td style="width:350px;text-align:left;">: '. $q['quotation']['reference'] .'</td>';
			$html .= '<th style="width:125px;text-align:left;">Nom de Client</th>';
			$html .= '<td style="text-align:left;">: '. $q['customer']['first_name'] .' '. $q['customer']['last_name'] .'</td>';
			$html .= '</tr>';
			$html .= '<tr>';
			$html .= '<th style="text-align:left;">Transporteur</th>';
			$html .= '<td style="text-align:left;">: '. $q['carrier']['carrier_name'] .'</td>';
			$html .= '<th style="text-align:left;">Téléhone</th>';
			$html .= '<td style="text-align:left;">: '. $q['customer']['mobile'] .'</td>';
			$html .= '</tr>';
			$html .= '<tr>';
			//$html .= '<th style="text-align:left;">Méthode de paiement</th>';
			//$html .= '<td style="text-align:left;">: '. $q['quotation']['payment_method'] .'</td>';
			$html .= '<th style="text-align:left;">Adresse de facturation</th>';
			$html .= '<td colspan="4" style="text-align:left;">: '. $q['quotation']['address_invoice'] .'</td>';
			$html .= '</tr>';
      ...
      $html .= '</table>';
		...
		if( $q['invoice']['informations'] != "" ){
			$html .= '<div style="margin-top:50px;">';
				$html .= '<p style="font-size:11px;"><strong>Informations: </strong>'. $q['invoice']['informations'] .'</p>';
			$html .= '</div>';
		}
		...
		$html .= '<div><br>';
			...
			if( $q['quotation']['more_info'] != "" ){
				$html .= '<strong>Informations complémentaires</strong>';
				$html .= '<p>'.$q['quotation']['more_info'].'</p>';
			}

			if( $q['quotation']['informations'] != "" ){
				$html .= '<strong>Divers</strong>';
				$html .= '<p style="margin:0px;">Informations complémentaires :</p>';
				$html .= '<p>'.$q['quotation']['informations'].'</p>';
			}

			$html .= '<p>Pour accepter, modifier, refuser ce devis ou pour nous contacter, rendez vous sur votre espace professionnel : ';
			$html .= '<a href="http://www.maroc-artiza.com/account/quotation">www.maroc-artiza.com/account/quotation</a><br>';
	...
	try
	{
	  ...
	  $mpdf->Output( $q['quotation']['reference'].'.pdf' , 'I' );
	}
