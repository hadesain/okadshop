 ...
 */
	...
	//we have id_order in url
	if ( empty($_GET['id_order']) ) return;

	//order
	$id_order = intval($_GET['id_order']);
	if ( intval($_GET['id_order']) <= 0 ) return;
	...
	//payment_choice
	$payment_choice = $o['order']['payment_choice'];
	if( $payment_choice != "" ){
		$methods = $common->select('payment_methodes', array('value'), "WHERE id IN($payment_choice)" );
	}
	if( $o['order']['id_payment_method'] != "0" ){
		$payment_method = $common->select('payment_methodes', array('value'), "WHERE id=".$o['order']['id_payment_method'] );
	}


	//currency
	$currency = $o['order']['currency_sign'];
	...
	$avoir 					 = $o['order']['avoir'];
	//carrier infos
	$shipping_costs  = number_format($o['carrier']['shipping_costs'], 2);
	...
	$html .= '<div style="width:1024px;margin:0px auto;">';
		$html .= '<table cellpadding="5" cellspacing="0" style="width:1024px;margin:0px auto;">';
			...
			$html .= '<h2 style="color:#44271A;">COMMANDE</h2>';
			$html .= '</th>';
			$html .= '</tr>';
		...
		$html .= '<table border="0" cellpadding="5" cellspacing="0" style="width:1024px;">';
			...
			$html .= '<td style="width:350px;text-align:left;">: '. $o['order']['reference'] .'</td>';
			$html .= '<th style="width:125px;text-align:left;">Nom de Client</th>';
			$html .= '<td style="text-align:left;">: '. $o['customer']['first_name'] .' '. $o['customer']['last_name'] .'</td>';
			$html .= '</tr>';
			$html .= '<tr>';
			$html .= '<th style="text-align:left;">Transporteur</th>';
			$html .= '<td style="text-align:left;">: '. $o['carrier']['carrier_name'] .'</td>';
			$html .= '<th style="text-align:left;">Téléhone</th>';
			$html .= '<td style="text-align:left;">: '. $o['customer']['mobile'] .'</td>';
			$html .= '</tr>';
			$html .= '<tr>';
			//$html .= '<th style="text-align:left;">Méthode de paiement</th>';
			//$html .= '<td style="text-align:left;">: '. $o['order']['payment_method'] .'</td>';
			$html .= '<th style="text-align:left;">Adresse de facturation</th>';
			$html .= '<td colspan="4" style="text-align:left;">: '. $o['order']['address_invoice'] .'</td>';
			$html .= '</tr>';
		...
		$html .= '<table cellpadding="10" cellspacing="0" style="width:1024px;margin:0px auto;border:1px solid #44271A;" id="products">';
			...
			$html .= '<tbody>';
				if( !empty($o['products']) ){
					foreach ($o['products'] as $key => $product) {
		        $html .= '<tr>';
      ...
      $html .= '</table>';
		...
		if( $o['invoice']['informations'] != "" ){
			$html .= '<div style="margin-top:50px;">';
				$html .= '<p style="font-size:11px;"><strong>Informations: </strong>'. $o['invoice']['informations'] .'</p>';
			$html .= '</div>';
		}
		...
		$html .= '<div><br>';
			...
			if( $o['order']['more_info'] != "" ){
				$html .= '<strong>Informations complémentaires</strong>';
				$html .= '<p>'.$o['order']['more_info'].'</p>';
			}

			if( $o['order']['informations'] != "" ){
				$html .= '<strong>Divers</strong>';
				$html .= '<p style="margin:0px;">Informations complémentaires :</p>';
				$html .= '<p>'.$o['order']['informations'].'</p>';
			}

			$html .= '<p>Pour accepter, modifier, refuser cette commande ou pour nous contacter, rendez vous sur votre espace professionnel : ';
			$html .= '<a href="http://www.maroc-artiza.com/account/history">www.maroc-artiza.com/account/history</a><br>';
	...
	try
	{
	  ...
	  $mpdf->Output( $o['order']['reference'].'.pdf' , 'I' );
	}
