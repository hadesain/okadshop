...
<?php 
		$product_in_promos = getProductPromos();
		if (!empty($product_in_promos)) {
			$condition = " p.id in ($product_in_promos)";
			$params = array(
...
<ol class="breadcrumb">
	...
	<li class="active"><?=l("Promotions", "artiza");?></li>
</ol>

<h1><?=l("Promotions", "artiza");?>
	<?php 
		$nbproduct = count(explode(',', $product_in_promos)); 
	?>
	<?php if (!$nbproduct || $nbproduct ==0 || $product_in_promos == null): ?>
		<span class="category-product-count"> Aucun produit.</span>
	<?php else: ?>
	<span class="category-product-count"> <?=l("Il y a", "artiza");?>  <?= $nbproduct; ?> <?=l("produits.", "artiza");?></span>
	<?php endif ?>
</h1>
