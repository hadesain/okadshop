...
class Database 
{
    ...
    static private function get_config_file($file_path)
    {
      try {
        if( file_exists( $file_path ) ){
          $file_data = file_get_contents( $file_path );
          //$data = json_decode( $file_data, true );
          if( !empty( $file_data ) ) return $file_data;
          return false;
        }
      } catch (Exception $e) {
        echo $e->getMessage();
    ...
    static public function getInstance() 
    {
        if(!self::$PDOInstance) { 
	        try {
            self::$PDOInstance = new PDO("mysql:host="._DB_SERVER_.";dbname="._DB_NAME_,_DB_USER_,_DB_PASS_,array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8",PDO::ATTR_ERRMODE=> PDO::ERRMODE_EXCEPTION ));
