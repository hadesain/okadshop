...
try {
  //customers count
  $customers = $common->select('users', array('COUNT(id) as count'), "WHERE id_group IN (2,3)");
...
foreach ($output as $stat) {
  $date = $stat['dates'];
  $q_count = $stat['q_count'] ? $stat['q_count'] : 0;
  $o_count = $stat['o_count'] ? $stat['o_count'] : 0;
  $i_count = $stat['i_count'] ? $stat['i_count'] : 0;
...
if( $total > 0 ){
  ...
  $donut_data = '[
                  {value: '. $q_rate .', label: "'. l("Devis", "admin") .'", formatted: "'. l("Au moins", "admin") .' '. number_format($q_rate, 2, '.', '') .' %" },
                  {value: '. $o_rate .', label: "'. l("Commandes", "admin") .'", formatted: "'. l("Au moins", "admin") .' '. number_format($o_rate, 2, '.', '') .' %" },
                  {value: '. $i_rate .', label: "'. l("Factures", "admin") .'", formatted: "'. l("Environ", "admin") .' '. number_format($i_rate, 2, '.', '') .' %" }
                ]';

}else{
  $q_rate = $o_rate = $i_rate = 0;
  $donut_data = '[{value: 0, label: "'. l("Vide", "admin") .'", formatted: "'. l("Pas de données disponibles", "admin") .'" }]';
...
<div><!--style="margin-right:25%"-->
  <div class="row" id="admin-home">
  	<div class="col-sm-12 padding0">
  		<div class="center-block">
  			<div class="jumbotron">
  			  <h2><?=l("Bonjour", "admin");?>, <?php echo $user_info['first_name']. " ". $user_info['last_name']; ?>.</h2>
  			  <p><?=l("Voici ce qui se passe avec votre magasin dès maintenant.", "admin");?></p>
  			</div>
              ...
              <div class="col-xs-4 col-sm-2">
                <div class="panel panel-default">
                  <div class="panel-body">
                    <label><?=l("Transporteurs", "admin");?></label>
      ...
      <div class="col-sm-12">
        <div id="dashaddons">
          <a href="?module=modules&page=positions">
            <i class="fa fa-plus"></i> <?=l("Ajoutez des modules à votre tableau de bord", "admin");?>
          </a>
        </div>
      </div>
    </div><!-- /.col-sm-9 -->
    <div class="col-sm-3">
      <div class="panel panel-default">
        <div class="panel-heading">
          <i class="fa fa-rss"></i> <?=l("ACTUALITÉS OKADSHOP", "admin");?></div>
        <div class="panel-body">
          <div id="os_news"></div>
          <center><a href="#" class="btn more_news"><?=l("Trouver plus d'actualités", "admin");?></a></center>
        </div>
      </div>
      <div class="panel panel-default">
        <div class="panel-heading">
          <i class="fa fa-refresh"></i> <?=l("MISES À JOUR", "admin");?></div>
        <div class="panel-body">
          <center>
            <p><?=l("Vous pouvez mettre à jour OkadShop", "admin");?> <span class="os_version"><?=_OS_VERSION_;?></span></p>
            <a href="?module=modules&slug=os-updates&page=update-core" class="btn btn-default" style="margin-top: -4px;"><?=l("Mettre à jour", "admin");?></a>
          </center>
        </div>
      </div>
  ...
  $(document).ready(function(){
    ...
    if( $("#data_month").length ){
      Morris.Bar({
        ...
        xkey: 'date',
        ykeys: ['d','o', 'f'],
        labels: [ '<?=l("Devis", "admin");?>','<?=l("Commandes", "admin");?>', '<?=l("Factures", "admin");?>'],
      });
    }
  ...
  function os_check_updates() {
    $.getJSON( "http://updates.okadshop.com/index.php", function( data ) {
      ...
      {
        var alert = '<div class="alert alert-success"><a href="?module=modules&slug=os-updates&page=update-core" class="btn btn-default pull-right" style="margin-top: 4px;"><?=l("Mettre à jour.", "admin");?></a><h4><?=l("OKADSHOP MISES À JOUR.", "admin");?></h4><p><?=l("Vous pouvez mettre à jour OkadShop", "admin");?> '+data.version+'</p></div>';
