...
<div class="top-menu padding0">
  <div class="top-menu-title">
    <h3><i class="fa fa-credit-card"></i> <?=l("Edition de la Commande", "admin");?>
    <span class="badge badge-default"><?php echo $o['order']['reference']; ?></span>
		<span class="badge">N°<?php echo sprintf("%06d", $o['order']['id']);?></span>
	</h3>
  </div>
  <a href="?module=orders" class="btn btn-default pull-right"><?=l("Términer", "admin");?></a>
</div><br>
...
<div class="col-sm-12 padding0" style="margin-bottom:10px;border-bottom:1px solid #A5245E;">
	<ul class="nav nav-tabs bg-white">
		<li class="active"><a data-toggle="tab" href="#step-1"><span class="number">1</span> <?=l("Produits", "admin");?></a></li>
		<li><a data-toggle="tab" href="#step-2"><span class="number">2</span> <?=l("Client", "admin");?></a></li>
		<!--li><a data-toggle="tab" href="#step-3"><span class="number">3</span> Paiement</a></li-->
		<li><a data-toggle="tab" href="#step-4"><span class="number">3</span> <?=l("Transporteur", "admin");?></a></li>

		<li class="pull-right">
			<?php if( $id_invoice > 0 ) : ?>
			<a target="_blank" href="../pdf/invoice.php?id_invoice=<?=$id_invoice;?>&id_customer=<?=$o['order']['id_customer'];?>" class="btn btn-success" style="border-radius: 0px;color: #fff;"><i class="fa fa-file-text"></i> <?=l("Apérçu la facture", "admin");?></a>
			<?php else : ?>
				<a target="_blank" class="btn btn-success gen_invoice" style="border-radius: 0px;color: #fff;"><i class="fa fa-cogs"></i> <?=l("Générer la facture", "admin");?></a>
			<?php endif; ?>
		</li>
		<li class="pull-right">
			<a target="_blank" href="../pdf/order.php?id_order=<?=$o['order']['id'];?>&id_customer=<?=$o['order']['id_customer'];?>" class="btn"><i class="fa fa-file-pdf-o"></i> <?=l("Apérçu la Commande", "admin");?></a>
		</li>
	</ul>
</div>
	...
	<div class="tab-pane active" id="step-1">
		<div class="panel panel-default">
				...
				<form class="form-horizontal" id="product_form" method="post" action="ajax/orders/form/product.php">
					...
					<thead>
						<tr>
							...
							<th width="190"><?=l("Référence", "admin");?></th>
							<th width="150"><?=l("Prix HT", "admin");?> (<?=$currency_sign;?>)</th>
							<th width="150"><?=l("Quantité", "admin");?></th>
							<th width="150"><?=l("Poids (Kg)", "admin");?></th>
							<th width="95"><?=l("Actions", "admin");?></th>
						</tr>
					</thead>
					<tbody>
						<tr>
			          ...
			          <select style="margin-top: 8px;" name="id_declinaisons" class="form-control hidden" id="based_attributs" disabled required>
									<option value="" selected><?=l("Sélectionnez une attribut", "admin");?></option>
								</select>
				      	...
				      	<button type="submit" class="btn btn-primary"><i class="fa fa-plus"></i> <?=l("Ajouter", "admin");?></button>
				      </td>
						</tr>
					</tbody>
					</table>
		    ...
		    <div class="panel-subheading">
					...
					<strong><?=l("Produits de la Commande", "admin");?></strong>
				</div>
				...
				<div class="col-sm-3 col-xs-12 pull-right padding0">
						<table class="table table-striped table-bordered" id="order_total">
							<tbody>
								...
								<tr>
									<th width="150"><?=l("Poids total", "admin");?></th>
		...
		<div class="panel panel-default">
			<div class="panel-body">
				<table id="customet" class="table table-striped table-bordered" cellspacing="0" width="100%">
					<thead>
						<tr>
							<th><?=l("N° Client", "admin");?></th>
							<th><?=l("Client", "admin");?></th>
							<th><?=l("Email", "admin");?></th>
							<th><?=l("Téléphone domicile", "admin");?></th>
							<th><?=l("Téléphone portable", "admin");?></th>
		...
		<div class="panel panel-default">
			<div class="panel-body">
				...
				<form class="form-horizontal" method="post" action="">
					...
					<thead>
						<tr>
							<th width="200"><?=l("Date de transaction", "admin");?></th>
							<th><?=l("Methode de paiment", "admin");?></th>
							<th><?=l("ID de transaction", "admin");?></th>
							<th width="150"><?=l("Montant payé", "admin");?></th>
							<th width="50"><?=l("Actions", "admin");?></th>
						</tr>
					</thead>
					<tbody>
						<tr>
			            ...
			            <?php endif; ?>
							...
							<td style="text-align:center;">
				      	<button type="submit" name="save_payment" class="btn btn-primary"><i class="fa fa-plus"></i> <?=l("Ajouter", "admin");?></button>
				      </td>
						</tr>
					</tbody>
					</table>
		    ...
		    <div class="panel-subheading">
					...
					<strong><?=l("Tranches payés", "admin");?></strong>
				</div>
			    ...
			    <table class="table table-striped table-bordered" cellspacing="0" width="100%">
			      <thead>
			        <tr style="background-color: #EFF9FD;border-bottom:2px solid #ddd;">
			          <th><?=l("ID", "admin");?></th>
			          <th><?=l("Date de transaction", "admin");?></th>
			          <th><?=l("Méthode de paiement", "admin");?></th>
			          <th><?=l("ID de transaction", "admin");?></th>
			          <th><?=l("Montant payé", "admin");?> (<?=$currency_sign;?>)</th>
			        </tr>
			      </thead>
			      <tbody>
		...
		<div class="panel panel-default">
			<div class="panel-body">
				...
				<form class="form-horizontal" id="carrier_form" method="post" action="ajax/orders/form/carrier.php">
					...
					<div class="form-group">
						<label class="col-md-3 control-label"><?=l("Délai de livraison", "admin");?></label>
						<div class="col-sm-2 right0">
							<div class="input-group">
								<span class="input-group-addon"><?=l("Entre", "admin");?></span>
						...
						<div class="col-sm-2 left0 right0">
							<div class="input-group" style="left:-2px;">
								<span class="input-group-addon"><?=l("Et", "admin");?></span>
			          ...
			          <option value="0" selected=""><?=l("Jours", "admin");?></option>
			          <option value="1" <?=($o['carrier']['delay_type']=="1") ? 'selected' : '';?>><?=l("Semaines", "admin");?></option>
			        </select>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label"><?=l("Délai de préparation", "admin");?></label>
						<div class="col-sm-2 right0">
							<div class="input-group">
								<span class="input-group-addon"><?=l("Entre", "admin");?></span>
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="weight_including_packing"><?=l("Poids total estimé emballage compris", "admin");?></label>  
						<div class="col-sm-3">
							<div class="input-group">
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="number_packages"><?=l("Nombre de colis estimé", "admin");?></label>  
						<div class="col-sm-3">
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="more_info"><?=l("Informations complémentaires", "admin");?></label>  
						<div class="col-sm-6">
					...
					<div class="form-group">
						<div class="col-sm-2 col-sm-offset-3">
							<button type="submit" class="btn btn-success"><i class="fa fa-refresh"></i> <?=l("Mise à jour", "admin");?></button>
						</div>
					</div>
