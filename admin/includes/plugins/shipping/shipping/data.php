...
if ( 
  ...
  $carrier_data = array(
    'id_tax' 						=> intval($_POST['shipping']['id_tax']),
		...
		'shipping_handling' => intval($_POST['shipping']['handling']),
		'is_free' 					=> intval($_POST['shipping']['is_free']),
		'shipping_method' 	=> intval($_POST['shipping']['method']),
		'range_behavior' 		=> intval($_POST['shipping']['range_behavior']),
		'range_inf' 				=> floatval($_POST['shipping']['range_inf']),
		'range_sup' 				=> floatval($_POST['shipping']['range_sup']),
				...
				foreach ($zones as $key => $zone) {
				  ...
				  if( ! $exist ){
				    ...
				    $common->save('carrier_zones', $zone_data);
				  	...
				  	$common->update('carrier_zones', array('fees' => $zone['fees'], 'active' => 1), "WHERE id_carrier=". $id_carrier ." AND id_zone=".$zone['id_zone'] );
 				  }
				}
				$zone_ids = substr_replace($zone_ids, "", -1);
				$common->update('carrier_zones', array('fees'=> null, 'active' => '0'), "WHERE id_carrier=". $id_carrier ." AND id_zone NOT IN (". $zone_ids .")" );
			}else{
  ...
  <div class="top-menu padding0">
    <div class="top-menu-title">
      <h3>
      	...
      	<?= (isset($id_carrier) && $id_carrier > 0) ? l("Modifier le Transporteur") : l("Ajouter un Transporteur", "admin");?>
      </h3>
    </div>
    <div class="top-menu-button">
    	<button type="button" class="btn btn-default" onclick="window.location='?module=shipping';"><?=l("Terminer", "admin");?></button>
      <button type="submit" name="<?= (isset($id_carrier) && $id_carrier > 0) ? 'update' : 'save';?>" class="btn btn-primary"><?=l("Sauvegarder et rester", "admin");?></button>
    </div>
  </div><br>
  ...
  <?php if(!empty($errors)) : ?>
	...
	<div class="col-sm-12 padding0" style="margin-bottom:10px;border-bottom:1px solid #A5245E;">
		<ul class="nav nav-tabs bg-white">
			<li><a data-toggle="tab" href="#step-1"><span class="number">1</span> <?=l("Paramètres généraux", "admin");?></a></li>
			<li><a data-toggle="tab" href="#step-2"><span class="number">2</span> <?=l("Destinations d'expédition et coûts", "admin");?></a></li>
			<li><a data-toggle="tab" href="#step-3"><span class="number">3</span> <?=l("Taille, poids et groupes associés", "admin");?></a></li>
			<li><a data-toggle="tab" href="#step-4"><span class="number">4</span> <?=l("Récapitulatif", "admin");?></a></li>
		</ul>
	</div>
	...
	<div class="tab-content col-sm-12 padding0">
		...
		<div class="tab-pane active" id="step-1">
			<div class="panel panel-default">
				<div class="panel-heading"><?=l("Paramètres généraux", "admin");?></div>
				<div class="panel-body">
					<div class="form-group">
					<label class="col-md-3 control-label"><?=l("Statut", "admin");?></label> 
						<div class="col-sm-3">
							<input type="checkbox" name="shipping[active]" class="switch" id="active" value="1" <?= ($shipping[0]['active']=="1") ? 'checked' : '';?> data-on-text="<?=l("activé", "admin");?>" data-off-text="<?=l("désactivé", "admin");?>" />
						</div>
						<div class="col-sm-3" style="position: absolute;left: 0px;">
							<span id="carrier_img">
								<?php if( $shipping[0]['logo'] != "" ) : ?>
					...
					<div class="form-group">
					  <label class="control-label col-lg-3"><?=l("Mode de livraison", "admin");?></label>
					  <div class="col-lg-3">
					    <select name="shipping[type]" class="form-control" id="type">
					      <option value="economic" selected><?=l("Economique (le moins cher)", "admin");?></option>
					      <option value="express" <?=( isset($shipping[0]['type']) && $shipping[0]['type']=="express" ) ? "selected" : "";?>><?=l("Express (le plus rapide)", "admin");?></option>
					    </select>
					  </div>
					</div>
					<div class="form-group">
					  <label class="control-label col-lg-3 required"><span class="label-tooltip" data-html="true" data-original-title="<?=l("Caractères autorisés : lettres, espaces et &quot;().-&quot;. Nom du transporteur tel qu'il apparaîtra durant la commande. Pour le retrait en boutique, mettez le chiffre 0 afin de remplacer le nom du transporteur par le nom de votre boutique.", "admin");?>" data-toggle="tooltip" title=""><?=l("Nom du transporteur *", "admin");?></span></label>
					  <div class="col-lg-3">
					    <input class="form-control" id="name" type="text" name="shipping[name]" value="<?php echo $shipping[0]['name'];?>" required>
					  </div>
					</div>
					<div class="form-group">
						<label class="control-label col-lg-3 required">
							<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="<?=l("L'estimation du délai de livraison sera affichée durant la commande.", "admin");?>"><?=l("Délai de livraison *", "admin");?></span>
						</label>
						<div class="col-lg-1">
							<input class="form-control" type="number" id="min_delay" name="shipping[min_delay]" value="<?php echo $shipping[0]['min_delay'];?>" placeholder="<?=l("min", "admin");?>" value="" required>
						</div>
						<div class="col-lg-1">
							<input class="form-control" type="number" id="max_delay" name="shipping[max_delay]" value="<?php echo $shipping[0]['max_delay'];?>" placeholder="<?=l("max", "admin");?>" value="" required>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-lg-3">
							<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="<?=l("Saisissez &quot;0&quot; pour le délai d'expédition le plus long, ou &quot;9&quot; pour le délai le plus court.", "admin");?>"><?=l("Vitesse", "admin");?></span>
						</label>
						<div class="col-lg-3">
							<input type="number" min="0" size="1" name="shipping[grade]" value="<?php echo $shipping[0]['grade'];?>" id="grade" placeholder="0" class="form-control">
						</div>
					</div>
					<div class="form-group">
					  <label class="control-label col-lg-3"><?=l("Logo", "admin");?></label>
					  <div class="col-lg-3">
							<input type="file" name="logo" id="attachement">
							<small><?=l("Format JPG, GIF, PNG. Taille du fichier 8.00 Mo max. Taille actuelle", "admin");?> <span id="carrier_logo_size"><?=l("indéfini", "admin");?></span>.</small>
						</div>
					</div>
					<div class="form-group">
					  <label class="control-label col-lg-3"><span class="label-tooltip" data-html="true" data-original-title="<?=l("URL pour le suivi des colis : placez le caractère &quot;@&quot; là où la référence doit apparaître. Elle sera automatiquement remplacée par le numéro de suivi.", "admin");?>" data-toggle="tooltip" title=""><?=l("URL de suivi", "admin");?></span></label>
					  <div class="col-lg-3">
					    <input class="form-control" name="shipping[url]" value="<?php echo $shipping[0]['url'];?>" id="url" type="text" placeholder="http://..">
					    <p class="help-block"><?=l('Par exemple : "http://example.com/track.php?num=@" avec "@" à l\'emplacement du numéro de suivi.');?></p>
					  </div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="description"><?=l("Informations complémentaires", "admin");?></label>  
						<div class="col-sm-6">
							<textarea name="shipping[description]" id="description" class="form-control summernote"><?php echo $shipping[0]['description'];?></textarea>
						</div>
					</div>

				</div><!--/ .panel-body -->
				<div class="panel-footer">
					<button type="button" class="btn btn-default" onclick="window.location='?module=shipping';"><?=l("Fermer", "admin");?></button>
					<button type="submit" name="<?= (isset($id_carrier) && $id_carrier > 0) ? 'update' : 'save';?>" class="btn btn-primary pull-right"><?=l("Sauvegarder et rester", "admin");?></button>
				</div><!--/ .panel-footer -->
			</div><!--/ .panel -->
		</div><!--/ #step-1 -->
		...
		<div class="tab-pane" id="step-2">
			...
			<div class="panel panel-default">
				<div class="panel-heading"><?=l("Destinations d'expédition et coûts", "admin");?></div>
				<div class="panel-body">

				<div class="col-sm-6">
					<div class="form-group">
						<label class="col-md-4 control-label"><?=l("Ajouter les frais de manutention", "admin");?></label> 
	          <div class="col-sm-3">
	            <input type="checkbox" name="shipping[handling]" value="1" <?= ($shipping[0]['shipping_handling']=="1") ? 'checked' : '';?>  class="switch" id="handling" data-on-text="<?=l("OUI", "admin");?>" data-off-text="<?=l("NON", "admin");?>" />
	          </div>
	        </div>
					<div class="form-group">
						<label class="col-md-4 control-label"><?=l("Frais de port offerts", "admin");?></label> 
	          <div class="col-sm-3">
	            <input type="checkbox" name="shipping[is_free]" value="1" <?= ($shipping[0]['is_free']=="1") ? 'checked' : '';?>  class="switch" id="isfree" data-on-text="<?=l("OUI", "admin");?>" data-off-text="<?=l("NON", "admin");?>" />
	          </div>
	        </div>
					<div class="form-group">
						<label class="control-label col-lg-4"><?=l("Facturation", "admin");?></label>
						<div class="col-lg-4">
							<div class="radio t">
								<label><input type="radio" name="shipping[method]" id="billing_price" <?= ($shipping[0]['shipping_method']=="0") ? 'checked' : '';?> value="0" checked><?=l("En fonction du prix total.", "admin");?></label>
							</div>
							<div class="radio t">
								<label><input type="radio" name="shipping[method]" id="billing_weight" <?= ($shipping[0]['shipping_method']=="1") ? 'checked' : '';?> value="1"><?=l("En fonction du poids total.", "admin");?></label>
							</div>
						</div>
					</div>
					<div class="form-group">
		        <label class="col-md-4 control-label"><?=l("Taxes", "admin");?></label> 
		        <div class="col-sm-4">
				...
				<div class="col-sm-6 col-lg-6 col-xs-12">
					<div class="form-group">
					  <div id="zone_ranges">
					    ...
					    <table class="table" id="zones_table" cellpadding="4">
					      <tbody>
					        <tr class="bg-gray">
					          <td class="range_type" width="300"><?=l("Sera appliquée lorsque le poids est", "admin");?></td>
					        ...
					        <tr class="bg-gray">
					          <td class="range_type"><?=l("Sera appliquée lorsque le poids est", "admin");?></td>
					        ...
					        <tr class="fees_all">
					          <td><span class="fees_all"><?=l("All", "admin");?></span></td>
					          <td style="">
					          ...
					          <td>
					            <div class="input-group">
					              ...
					              <input name="all" value="<?php echo $shipping[0]['all'];?>" autocomplete="off" class="form-control" type="number" min="0" step="0.01" placeholder="0.00">
					            </div>
					          </td>
					        </tr>
					...
					<div class="new_range">
						<a target="_blank" class="btn btn-default" href="?module=zones&action=add" id="add_new_range" onclick="add_new_range();return false;"><?=l("Ajouter une nouvelle tranche", "admin");?></a>
					</div>
				</div><!--/ .col-sm-6  -->
			...
			<div class="alert alert-info">
				...
				<?=l("Vous devez enregistrer ce transport avant d'y ajouter des destinations d'expédition et coûts.", "admin");?>
			</div>
			<?php endif; ?>
		</div><!--/ #step-2 -->
		...
		<div class="tab-pane" id="step-3">
			<?php if(isset($id_carrier) && $id_carrier > 0) : ?>
			<div class="panel panel-default">
				<div class="panel-heading"><?=l("Taille, poids et groupes associés", "admin");?></div>
				<div class="panel-body">

					<div class="form-group">
					  <label class="control-label col-lg-3">
					  	<span class="label-tooltip" data-html="true" data-original-title="<?=l("Largeur maximale gérée par ce transporteur. Inscrivez &quot;0&quot; ou laissez vide pour ignorer cette option. La valeur doit être un entier.", "admin");?>" data-toggle="tooltip" title=""><?=l("Largeur maximum du paquet (cm)", "admin");?></span>
					  </label>
					  <div class="col-lg-3">
					...
					<div class="form-group">
					  <label class="control-label col-lg-3">
					  	<span class="label-tooltip" data-html="true" data-original-title="<?=l("Hauteur maximale gérée par ce transporteur. Inscrivez &quot;0&quot; ou laissez vide pour ignorer cette option. La valeur doit être un entier.", "admin");?>" data-toggle="tooltip" title=""><?=l("Hauteur maximum du paquet (cm)", "admin");?></span>
					  </label>
					  <div class="col-lg-3">
					...
					<div class="form-group">
					  <label class="control-label col-lg-3">
					  	<span class="label-tooltip" data-html="true" data-original-title="<?=l("Profondeur maximale gérée par ce transporteur. Inscrivez &quot;0&quot; ou laissez vide pour ignorer cette option. La valeur doit être un entier.", "admin");?>" data-toggle="tooltip" title=""><?=l("Profondeur maximum du paquet (cm)", "admin");?></span>
					  </label>
					  <div class="col-lg-3">
					...
					<div class="form-group">
					  <label class="control-label col-lg-3">
					  	<span class="label-tooltip" data-html="true" data-original-title="<?=l("Poids maximal géré par ce transporteur. Inscrivez &quot;0&quot; ou laissez vide pour ignorer cette option.", "admin");?>" data-toggle="tooltip" title=""><?=l("Poids maximum du paquet (kg)", "admin");?></span>
					  </label>
					  <div class="col-lg-3">
					...
					<div class="form-group">
					  <label class="control-label col-lg-3"><span class="label-tooltip" data-html="true" data-original-title="<?=l("Cochez tous les groupes qui ont accès à ce transporteur.", "admin");?>" data-toggle="tooltip" title=""><?=l("Accès des groupes", "admin");?></span></label>
					  <div class="col-lg-9">
					    <div class="row">
					      <div class="col-lg-6">
					        <table class="table table-bordered">
					          <thead>
					            <tr>
					              ...
					              <th><label for="checkme"><?=l("Nom du groupe", "admin");?></label></th>
					            </tr>
					          </thead>
					          <tbody>
			...
			<div class="alert alert-info">
				<h4><?=l("Il y a 1 avertissement.", "admin");?></h4>
				<?=l("Vous devez enregistrer ce transport avant d'y ajouter la taille, poids et groupes associés.", "admin");?>
			</div>
			<?php endif; ?>
		</div><!--/ #step-3 -->
		...
		<div class="tab-pane" id="step-4">
			<?php if(isset($id_carrier) && $id_carrier > 0) : ?>
			<div class="panel panel-default">
				<div class="panel-heading"><?=l("Informations de transporteur", "admin");?></div>
				<div class="panel-body">
					<?php if(isset($id_carrier) && $id_carrier > 0) : ?>
						<?php $key = $common->array_searching($taxes, 'id', $shipping[0]['id_tax']); ?>
						<?php $unit = ( $shipping[0]['shipping_method'] == "1" ) ? 'kg' : '€';?>
						<p>
							<?=l("Le transporteur est", "admin");?> <strong><?= ($shipping[0]['is_free']=="1") ? l("Offert", "admin") : l("payant", "admin"); ?></strong> <?=l("et le délai de livraison affiché est :", "admin");?> <strong><?= ($shipping[0]['min_delay']) ? $shipping[0]['min_delay'] : "0"; ?> à <?= (isset($shipping[0]['max_delay'])) ? $shipping[0]['max_delay'] : "0"; ?> <?=l("jours.", "admin");?></strong><br>
							<?=l("Le coût d'envoi est calculé", "admin");?> <strong><?= ($shipping[0]['shipping_method']=="1") ? l("en fonction du poids") : l("en fonction du prix", "admin"); ?></strong>, <?=l("et la règle de taxe", "admin");?> <strong><?= ($shipping[0]['id_tax'] > 0) ? $taxes[ $key ]['name'] : l("Aucune taxe", "admin"); ?></strong> <?=l("est appliquée", "admin");?> <br>
							<span class="is_free"><?=l("Ce transporteur peut livrer des commandes de", "admin");?> <strong><?=$shipping[0]['range_inf'].' '.$unit;?> à <?=$shipping[0]['range_sup'].' '.$unit;?>.</strong></span> <?=l("Si la commande est hors de cette tranche, le comportement est défini comme : prendre la tranche la plus grande.", "admin");?> <br>
							<br><?=l("Ce transporteur sera proposé pour les zones :", "admin");?><br>
							...
							<?=l("Et il sera proposé pour les groupes de clients suivants :", "admin");?>
							<ul style="margin-left:30px;">
								<?php
					...
					<?php else: ?>
						<div class="alert alert-info">
							<h4><?=l("Il y a 1 avertissement.", "admin");?></h4>
							<?=l("Vous devez enregistrer ce transport avant de percevoir la Récapitulatif.", "admin");?>
						</div>
					<?php endif; ?>
				</div><!--/ .panel-body -->
				<div class="panel-footer">
			...
			<div class="alert alert-info">
				<h4><?=l("Il y a 1 avertissement.", "admin");?></h4>
				<?=l("Vous devez enregistrer ce transport avant d'afficher les informations de transporteur.", "admin");?>
			</div>
			<?php endif; ?>
		</div><!--/ #step-4 -->
...
$(document).ready(function(){
  ...
  $('#zones_table input[type="checkbox"], #zones_table input[type=number]').on('change', function(){
  	var json = "";
  	$('#zones_table input[name="zones[]"]').each(function () {
		  if ( $(this).is(":checked") ) {
		  	json += ('{"id_zone":"'+ $(this).val() +'","fees":"'+ $(this).closest('tr').find('input[type=number]').val() +'"},');
	    }
		});
		json = "["+ json.slice(0,-1) +"]";
