...
<div class="top-menu padding0">
  <div class="top-menu-title">
    <h3><i class="fa fa-list"></i> <?=l("Groupe", "admin");?></h3>
  </div>
</div><br>

<div class="panel panel-default">
<form class="form-horizontal" id="ug_form" method="post" action="ajax/users/groups/form.php">
  <div class="panel-heading"><?=l("Modifier le groupe", "admin");?></div>
  <div class="panel-body">

    <input type="hidden" name="id_ug" value="<?=(isset($id_ug)) ? $id_ug : '0';?>" id="id_ug">
    <div class="form-group">
      <label class="control-label col-lg-3"><?=l("Nom de groupe *", "admin");?></label>
      <div class="col-lg-4">
        <input type="text" name="name" id="name" value="<?=(isset($ug[0]['name'])) ? $ug[0]['name'] : "";?>" class="form-control" required autofocus>     
      </div>
    </div>
    <div class="form-group">
      <label class="control-label col-lg-3" for="active"><?=l("Activé", "admin");?></label>
      <div class="col-lg-4">
        <input type="checkbox" name="active" class="active" id="active" value="1" <?=(isset($ug[0]['active']) && $ug[0]['active']=="1") ? 'checked' : '';?> data-on-text="<?=l("OUI", "admin");?>" data-off-text="<?=l("NON", "admin");?>" />
      </div>
    </div>
    
  </div><!--/ .panel-body -->
  <div class="panel-footer">
    <a href="?module=groups" class="btn btn-default"><?=l("Fermer", "admin");?></a>
