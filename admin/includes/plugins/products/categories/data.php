...
<div class="top-menu padding0">
  <div class="top-menu-title">
    <h3><i class="fa fa-sitemap"></i> <?=l("Catégorie", "admin");?></h3>
  </div>
  <a href="?module=categories" class="btn btn-primary pull-right"><?=l("Liste des Catégories", "admin");?></a>
</div><br>

<div class="panel panel-default">
<form class="form-horizontal" id="c_form" method="post" action="ajax/categories/form.php">
  <div class="panel-heading"><?=( isset($id_c) ) ? l("Modifier la Catégorie", "admin") : l("Nouvelle Catégorie", "admin");?></div>
  <div class="panel-body">

    <input type="hidden" name="id_c" value="<?=(isset($id_c)) ? $id_c : '0';?>" id="id_c">
    <div class="form-group">
      <label class="control-label col-lg-3"><?=l("Nom *", "admin");?></label>
      <div class="col-lg-4">
        <input type="text" name="name" id="name" value="<?=(isset($c[0]['name'])) ? $c[0]['name'] : "";?>" class="form-control" required autofocus>     
      </div>
    </div>
    <div class="form-group">
      <label class="control-label col-lg-3"><?=l("Catégorie Parent *", "admin");?></label>
      <div class="col-lg-4">
        <select name="parent" class="form-control" id="parent">
        <!--option value="0"><?//=l("Accueil", "admin");?></option-->
        <?php
        if( !empty($categories) )
        {
          ...
          foreach ($categories as $key => $category) {
            if( $category["id"] != $id_current_c )
            {
              ...
              echo '<option value="'.$category["id"].'" '.$selected.'>'. l( $category["name"], "admin") .'</option>';
            }
          }
        }
        ?>
    ...
    <div class="form-group">
      <label class="control-label col-lg-3"><?=l("Affichée", "admin");?></label>
      <div class="col-lg-4">
        <input type="checkbox" name="hidden" class="active" id="hidden" value="0" <?=(isset($c[0]['hidden']) && $c[0]['hidden']=="0") ? 'checked' : '';?> data-on-text="<?=l("OUI", "admin");?>" data-off-text="<?=l("NON", "admin");?>" />
      </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      <label class="control-label col-lg-3"><?=l("Image de la catégorie", "admin");?></label>
      <div class="col-sm-3">
    ...
    <div class="form-group">
      <div class="col-sm-2 col-sm-offset-3" id="c_logo">
        ...
        if( isset($c[0]['image_cat']) && $c[0]['image_cat'] != "" ){
          echo '<img src="../files/category/'.$c[0]['image_cat'].'" class="img-thumbnail" width="150">';
        }
        ?>
      </div>
    </div>
    <div class="form-group">
      <label class="control-label col-lg-3"><?=l("Balise titre", "admin");?></label>
      <div class="col-lg-6">
    ...
    <div class="form-group">
      <label class="control-label col-lg-3"><?=l("Meta mots-clés", "admin");?></label>
      <div class="col-lg-6">
        <input type="text" name="meta_keywords" id="tags" value="<?=(isset($c[0]['meta_keywords'])) ? $c[0]['meta_keywords'] : "";?>" class="form-control">     
      </div>
    </div>
    <div class="form-group">
      <label class="control-label col-lg-3"><?=l("URL simplifiée", "admin");?></label>
      <div class="col-lg-6">
  ...
  <div class="panel-footer">
    <a href="?module=categories" class="btn btn-default"><?=l("Fermer", "admin");?></a>
