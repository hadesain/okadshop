...
if(isset($_GET['id']) && $_GET['id'] > 0 && isset($_GET['action']) && $_GET['action'] == 'edit'){
	$id_product = $os_product->getProductByID( $_GET['id'], $id_user );
...
if ( $_SERVER['REQUEST_METHOD'] == 'POST' ){
	...
	if(isset($_POST['associated']) && $_POST['associated'] == "insert" )
	{
		...
		if( $save ){
			array_push($success, l("Le produit associé a été ajouter.", "admin") );
		}
	}
	...
	if( isset($_POST['assoc_delete']) )
	{
		...
		if( $id_assoc > 0 ){
			...
			if( $delete ){
				array_push($success, l("Le produit associé a été supprimer.", "admin") );
			}
		}
	}
...
*/
	...
	if(isset($_POST['update']) && !empty($_POST['product']['name'])){
		...
		if(isset($_POST['product']['categories'])){
			...
			$categories = array();
			$os_product->insertAssociations($id_product,$categories);
			array_push($errors, l("Le produit doit avoir au moin une seul catégorie.", "admin") );
		}
		...
		if(isset($_POST['id_image']) && isset($_POST['image_name']) && !empty($_POST['image_name'])){
			...
			if(isset($id_image) && $id_image > 0){
				...
				if( $copy )
				{
					...
					$newfile = "../files/products/".$id_product ."/dashboard-".$id_product.".jpg";//. $os_product->get_file_extension($_POST['image_name']);
					if( file_exists($file) )
					{
						if (!copy($file, $newfile)) {
						  array_push($errors, l("failed to copy.", "admin") );
						}
					}else{
						array_push($errors, l("Le fichier [$file] n'exist pas.", "admin") );
					}
				}
			}
		}
		...
		if(
			...
			array_push($errors, l("Le Nom du fichier est obligatoire.", "admin") );
		}

		array_push($success, l("Le produit a été mise à jour.", "admin") );
...
if(isset($id_product) && $id_product > 0){
	//Product Data
	$infos = $os_product->getProductData($id_product,$id_user);
  ...
  <div class="top-menu padding0">
    <div class="top-menu-title">
      <h3>
      	...
      	<?= (isset($id_product) && $id_product > 0) ? l("Modifier les informations de Produit", "admin") : l('Ajouter un Produit');?>
      </h3>
    </div>
    <div class="top-menu-button">
    	<button type="button" class="btn btn-default" onclick="window.location='?module=products';"><?=l("Terminer", "admin");?></button>
      <button type="submit" name="<?= (isset($id_product) && $id_product > 0) ? 'update' : 'save';?>" class="btn btn-primary"><?=l("Sauvegarder et rester", "admin");?></button>
    </div>
  </div><br>
	...
	<div class="col-sm-2 padding0">
		<ul class="nav nav-pills nav-stacked">
			<li class="active"><a data-toggle="pill" href="#Product"><?=l("Informations de Produit", "admin");?></a></li>
			<li><a data-toggle="pill" href="#Associated"><?=l("Produits associés", "admin");?></a></li>
			<li><a data-toggle="pill" href="#Prices"><?=l("Prix et Règle de taxe", "admin");?></a></li>
			<li><a data-toggle="pill" href="#Seo"><?=l("Référencement - SEO", "admin");?></a></li>
			<li><a data-toggle="pill" href="#Associations"><?=l("Catégories et Associations", "admin");?></a></li>
			<li><a data-toggle="pill" href="#Declinaisons"><?=l("Déclinaisons", "admin");?></a></li>
			<li><a data-toggle="pill" href="#Quantites"><?=l("Quantités", "admin");?></a></li>
			<li><a data-toggle="pill" href="#Images"><?=l("Images", "admin");?></a></li>
			<li><a data-toggle="pill" href="#Characteristics"><?=l("Caractéristiques", "admin");?></a></li>
			<li><a data-toggle="pill" href="#Attachments"><?=l("Documents joints", "admin");?></a></li>
		</ul>
	</div>
		...
		<div class="tab-content">
			...
			<div class="panel panel-default tab-pane fade in active" id="Product">
				<div class="panel-heading">
					<?=l("Informations de Produit", "admin");?>
				</div>
				<div class="panel-body">
					<div class="form-group">
						<div class="alert alert-info">
							<b>*</b> <?=l("Vous ne pouvez pas avoir de déclinaisons avec un produit virtuel.", "admin");?><br>
							<b>*</b> <?=l("Vous ne pouvez pas utiliser de déclinaisons dans un pack.", "admin");?>
						</div>
						<label class="col-md-3 control-label" for="type"><?=l("Type", "admin");?></label>  
						<div class="col-md-6">
							<div class="radio">
								<label for="simple">
									<input type="radio" name="product[type]" id="simple" value="0" checked="checked">
									<?=l("Produit standard", "admin");?>
								</label>
							</div>
							<div class="radio">
								<label for="pack">
									<input type="radio" name="product[type]" id="pack" <?=(isset($infos['type']) && $infos['type'] === "1") ? 'checked="checked"' : '';?> value="1"> <?=l("Pack de produits existants", "admin");?>
								</label>
							</div>
							<div class="radio">
								<label for="virtual">
									<input type="radio" name="product[type]" id="virtual" <?=(isset($infos['type']) && $infos['type'] === "2") ? 'checked="checked"' : '';?> value="2">
									<?=l("Produit dématérialisé (services, réservations, produits téléchargeables, etc.)", "admin");?>
								</label>
							</div>
						</div>
					</div><hr>
					<div class="form-group">
						<label class="col-md-3 control-label" for="Name"><?=l("Nom", "admin");?></label>  
						<div class="col-md-4">
							<input id="Name" name="product[name]" type="text" class="form-control" value="<?=(isset($infos['name'])) ? $infos['name'] : '';?>" required>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="reference"><?=l("Référence", "admin");?></label>  
						<div class="col-md-5">
						<input id="reference" name="product[reference]" type="text" class="form-control" value="<?=(isset($infos['reference'])) ? $infos['reference'] : '';?>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="reference"><?=l("Code-barres EAN-13 ou JAN", "admin");?></label>  
						<div class="col-md-3">
						<input id="ean13" name="product[ean13]" type="text" class="form-control" value="<?=(isset($infos['ean13'])) ? $infos['ean13'] : '';?>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="upc"><?=l("Code-barres UPC", "admin");?></label>  
						<div class="col-md-3">
						<input id="upc" name="product[upc]" type="text" class="form-control" value="<?=(isset($infos['upc'])) ? $infos['upc'] : '';?>">
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="upc"><?=l("État", "admin");?></label>  
						<div class="col-md-3">
							<select name="product[product_condition]" class="form-control" id="condition">
								<option value="new" selected><?=l("Nouveau", "admin");?></option>
								<option value="used" <?=(isset($infos['product_condition']) && $infos['product_condition'] == 'used') ? 'selected' : '';?>><?=l("Utilisé", "admin");?></option>
								<option value="refurbished" <?=(isset($infos['product_condition']) && $infos['product_condition'] == 'refurbished') ? 'selected' : '';?>><?=l("Reconditionné", "admin");?></option>
							</select>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="cdate"><?=l("Date de création", "admin");?></label>  
						<div class="col-sm-8">
					...
					<hr>
					<div class="form-group">
						<label class="col-md-3 control-label" ><?=l("Status", "admin");?></label>  
						<div class="col-md-3">
							<label for="enabled">
								<input type="radio" name="product[active]" value="1" id="enabled" checked> <?=l("Activé", "admin");?>
							</label>
							<label for="disabled">
								<input type="radio" name="product[active]" value="0" <?=(isset($infos['active']) && $infos['active'] === "0") ? 'checked' : '';?> id="disabled"> <?=l("Désactivé", "admin");?>
							</label>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="short_description"><?=l("Résumé", "admin");?></label>
						<div class="col-md-8">                     
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="long_description"><?=l("Description", "admin");?></label>
						<div class="col-md-8">      
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="tags"><?=l("Mots-clés (Tags)", "admin");?></label>  
						<div class="col-md-6">
							<input value="<?= (isset($tag_list)) ? $tag_list : '';?>" id="tags" name="tags[]" type="text" class="form-control">
							<small><?=l("Chaque mot-clé doit être suivi d'une virgule. Les caractères suivants sont interdits : !<;>;?=+#\"°{}_$%.", "admin");?></small>
						</div>
					</div>
				</div><!--/ .panel-body -->
				<div class="panel-footer">
					<button type="button" class="btn btn-default" onclick="window.location='?module=products';"><?=l("Fermer", "admin");?></button>
					<button type="submit" name="<?= (isset($id_product) && $id_product > 0) ? 'update' : 'save';?>" class="btn btn-primary pull-right"><?= (isset($id_product) && $id_product > 0) ? l("Mise à jour", "admin") : l("Sauvegarder et rester", "admin");?></button>
				</div><!--/ .panel-footer -->
			</div><!--/ .panel -->
			...
			<div class="tab-pane fade" id="Associated">
				...
				<div class="panel panel-default">
					<div class="panel-heading"><?=l("Produits associés", "admin");?></div>
					<div class="panel-body">
						<div class="form-group">
							<label class="col-md-3 control-label" for="associated"><?=l("Produit associé", "admin");?></label>  
							<div class="col-md-4">
								<select class="form-control" name="assoc[id_product]"><!-- chosen -->
									<option value="" selected><?=l("Sélectionnez un produit", "admin");?></option>
			            ...
			            <?php endforeach;?>
							...
							<div class="col-sm-1 left0">
								<button type="submit" name="associated" value="insert" class="btn btn-primary"><i class="fa fa-plus"></i> <?=l("Ajouter", "admin");?></button>
							</div>
						</div>
						...
						<div class="col-sm-5 col-sm-offset-3">
							<table class="table">
								<thead>
									<tr>
										<th style="width:20px;"><?=l("ID", "admin");?></th>
										<th style="text-align:center;width:70px;"><?=l("Image", "admin");?></th>
										<th><?=l("Nom de produit", "admin");?></th>
										<th><?=l("Prix HT", "admin");?></th>
										<th><?=l("Action", "admin");?></th>
									</tr>
								</thead>
								<tbody>
				...
				<?php else: ?>
				<div class="alert alert-info">
					<h4><?=l("Il y a 1 avertissement.", "admin");?></h4>
					<?=l("Vous devez enregistrer ce produit avant d'y ajouter des caractéristiques.", "admin");?>
				</div>
				<?php endif; ?>
			</div><!--/ .tab-pane -->
			...
			<div class="panel panel-default tab-pane fade" id="Prices">
				<div class="panel-heading">
					<?=l("Ajouter des Prix", "admin");?>
				</div>
				<div class="panel-body">
					<div class="form-group">
						<label class="col-md-3 control-label" for="buy_price"><?=l("Prix d'achat (unitaire)", "admin");?></label>  
						<div class="col-md-6">
							<div class="input-group">
								<div class="input-group-addon"><?=l("Dhs", "admin");?></div>
								...
								<div class="input-group-addon"><?=l("HT", "admin");?></div>
							</div>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="sell_price"><?=l("Prix de vente (unitaire)", "admin");?></label>  
						<div class="col-md-6">
							<div class="input-group">
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="wholesale_price"><?=l("Prix du lot", "admin");?></label>  
						...
						<div class="col-md-3 left0">
							<div class="input-group">
								<div class="input-group-addon"><?=l("Par", "admin");?></div>
								...
								<div class="input-group-addon"><?=l("(Pièces)", "admin");?></div>
							</div>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-3 control-label" for="loyalty_points"><?=l("Points de fidélité", "admin");?></label>  
						<div class="col-sm-4">
							<div class="input-group">
					      ...
					      <span class="input-group-btn">
					        <select class="btn loyalty_state">
					          <option value="0"><?=l("Activer", "admin");?></option>
					          <option value="1" <?=($infos['loyalty_points']=="0") ? 'selected' : '';?>><?=l("Désactiver", "admin");?></option>
					        </select>
					      </span>
					    </div>
						</div>
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="id_tax"><?=l("Règle de taxe", "admin");?></label>  
						<div class="col-md-6">
							<select class="form-control" id="id_tax" name="product[id_tax]">
	            	<option value="0" selected><?=l("Aucune taxe", "admin");?></option>
		            <?php if( !empty($taxes) ) : ?>
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="discount"><?=l("Remise", "admin");?></label>  
						<div class="col-md-4">
							<div class="input-group">
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="weight"><?=l("Colisage", "admin");?></label>  
						<div class="col-md-4">
							<div class="input-group">
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="weight"><?=l("Poids", "admin");?></label>  
						<div class="col-md-4">
							<div class="input-group">
								...
								<span class="input-group-addon"><?=l("KG", "admin");?></span>
							</div>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="height"><?=l("Hauteur", "admin");?></label>  
						<div class="col-md-4">
							<div class="input-group">
								...
								<span class="input-group-addon"><?=l("CM", "admin");?></span>
							</div>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label" for="width"><?=l("Largeur", "admin");?></label>  
						<div class="col-md-4">
							<div class="input-group">
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="depth"><?=l("Profondeur", "admin");?></label>  
						<div class="col-md-4">
							<div class="input-group">
								...
								<span class="input-group-addon"><?=l("m", "admin");?></span>
							</div>
						</div>
					</div>
				</div><!--/ .panel-body -->
			...
			<div class="panel panel-default tab-pane fade" id="Seo">
				<div class="panel-heading"><?=l("RÉFÉRENCEMENT - SEO", "admin");?></div>
				<div class="panel-body">
					<div class="form-group">
						<label class="col-md-3 control-label" for="meta_title"><?=l("Balise titre", "admin");?></label>  
						<div class="col-md-6">
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="meta_description"><?=l("Meta description", "admin");?></label>  
						<div class="col-md-6">
					...
					<div class="form-group">
						<label class="col-md-3 control-label" for="link_rewrite"><?=l("URL simplifiée", "admin");?></label>  
						<div class="col-md-6">
							<div class="input-group">
			...
			<div class="panel panel-default tab-pane fade" id="Associations">
				<div class="panel-heading">
					<?=l("Associations", "admin");?>
				</div>
				<div class="panel-body">
					<div class="form-group">
						<label class="col-md-3 control-label" for="sell_price"><?=l("Catégories associées", "admin");?></label>  
						<div class="col-md-6">
							<div class="assoc_categories">
								<div class="panel panel-custom" style="border-bottom: 0px none;margin-bottom: 0px;">
									<div class="panel-heading">
										<a onclick="colapse_all(); return false;" class="btn btn-default"><i class="fa fa-minus-square"></i> <?=l("Réduire tout", "admin");?></a>
										<a onclick="develope_all(); return false;" class="btn btn-default"><i class="fa fa-minus-square"></i> <?=l("Ouvrire tout", "admin");?></a>
										<a onclick="checkAllCategories(); return false;" class="btn btn-default"><i class="fa fa-check-square"></i> <?=l("Tout cocher", "admin");?></a>
										<a onclick="uncheckAllCategories(); return false;" class="btn btn-default"><i class="fa fa-square-o"></i> <?=l("Tout décocher", "admin");?></a>
									</div>
									<div class="panel-body">
					...
					<div class="form-group">
					  ...
					  </div><label class="control-label col-lg-2" for="id_category_default"><?=l("Catégorie par défaut", "admin");?></label>
					  <div class="col-lg-5">
			...
			<?php if(isset($id_product) && $id_product > 0) : ?>
				<div class="panel panel-default">
					<div class="panel-heading"><?=l("Ajouter ou modifier des déclinaisons de produit", "admin");?></div>
					<div class="panel-body">
						<!--div class="alert alert-info">
						...
						<div class="panel panel-default hidden" id="dec-form" style="border-bottom: 2px solid #ddd;">
							<div class="panel-heading">
								<button type="submit" name="<?= (isset($id_product) && $id_product > 0) ? 'update' : 'save';?>" class="btn btn-primary pull-right"><?=l("Sauvegarder la déclinaison", "admin");?></button>
								<?=l("Déclinaisons de produit", "admin");?>
							</div>
							<div class="panel-body">
								...
								<div class="form-group">
									<label class="col-md-3 control-label" for="attribute_group"><?=l("Attribut", "admin");?></label>  
									<div class="col-md-3">
								...
								<div class="form-group">
									<label class="col-md-3 control-label" for="values_group"><?=l("Valeur", "admin");?></label>  
									<div class="col-md-6">
										<select class="form-control" id="values_group" disabled></select>
									</div>
									<div class="col-md-3">
										<a type="button" class="btn btn-default btn-block" onclick="add_attr();"><i class="fa fa-plus-square"></i> <?=l("Ajouter", "admin");?></a>
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-md-3">
										<a type="button" class="btn btn-default btn-block add_attr" onclick="del_attr();"><i class="fa fa-minus-square"></i> <?=l("Supprimer", "admin");?></a>
									</div>
								</div>
								<hr>
								<div class="form-group">
									<label class="col-md-3 control-label" for="attr_reference"><?=l("Référence", "admin");?></label>  
									<div class="col-md-5">
									 <input id="attr_reference" name="declinaison[reference]" type="text" class="form-control" value="">
									</div>
								</div>
								<div class="form-group">
									<label class="col-md-3 control-label" for="attr_ean13"><?=l("Code-barres EAN-13 ou JAN", "admin");?></label>  
									<div class="col-md-3">
										<input id="attr_ean13" name="declinaison[ean13]" type="text" class="form-control" value="">
									</div>
								</div>
								<div class="form-group">
									<label class="col-md-3 control-label" for="attr_upc"><?=l("Code-barres UPC", "admin");?></label>  
									<div class="col-md-3">
								...
								<hr>
								<div class="form-group">
									<label class="col-md-3 control-label" for="sell_price"><?=l("Prix de vente HT", "admin");?></label>  
									<div class="col-md-3">
										<input id="attr_sell_price" name="declinaison[sell_price]" type="number" min="0" class="form-control" value="">
									</div>
								</div>
								<div class="form-group">
									<label class="col-md-3 control-label" for="price_impact"><?=l("Impact sur le prix", "admin");?></label>  
									<div class="col-md-3">
										<select class="form-control" name="declinaison[price_impact]" id="price_impact" onchange="displayOrHide(this,'attribute_price')">
											<option value="0"><?=l("Aucun", "admin");?></option>
											<option value="1"><?=l("Augmentation", "admin");?></option>
											<option value="2"><?=l("Diminution", "admin");?></option>
										</select>
									</div>
									<div class="input-group col-md-3 hidden" id="attribute_price">
										<span class="input-group-addon"><?=l("TTC", "admin");?></span>
										<input class="form-control" type="number" min="0" name="declinaison[price]" value="" placeholder="0.00">
									</div>
								</div>
								<div class="form-group">
									<label class="col-md-3 control-label" for="weight_impact"><?=l("Impact sur le poids", "admin");?></label>  
									<div class="col-md-3">
									...
									<div class="input-group col-md-3 hidden" id="attribute_weight">
										<span class="input-group-addon" style="width: 53px;"><?=l("KG", "admin");?></span>
										<input class="form-control" type="number" min="0" name="declinaison[weight]" value="" placeholder="0.00">
									</div>
								</div>
								<div class="form-group">
									<label class="col-md-3 control-label" for="unit_impact"><?=l("Impact sur le prix unitaire", "admin");?></label>  
									<div class="col-md-3">
								...
								<div class="form-group">
									<label class="control-label col-lg-3" for="attribute_default"><?=l("Image", "admin");?></label>
									<div class="col-lg-9">
											...
											<li id="<?=$image['id'];?>">
												...
												<div class="text-center">
													<span class="btn btn-default">
														<input type="checkbox" value="<?=$image['id'];?>" name="declinaison[images][]">
													</span>
												</div>
											</li>
											<?php endforeach; ?>
								...
								<div class="form-group">
									<label class="control-label col-lg-3"><?=l("Par défaut", "admin");?></label>
									<div class="col-lg-9">
										<p class="checkbox">
										<label for="default_dec">
											<input type="checkbox" name="declinaison[default_dec]" id="default_dec" value="1">
											<?=l("Faire de cette déclinaison celle par défaut pour ce produit.", "admin");?>
										</label>
										</p>
									</div>
								</div>
							...
							<div class="panel-footer">
								<a href="javascript:;" type="reset" id="edit-reset" class="btn btn-default edit-reset"> <i class="fa fa-undo"></i> <?=l("Fermer la modification", "admin");?></a>
								<span class="pull-right">
									<a href="javascript:;" class="btn btn-danger btn-sm add_new_dec"><?=l("Nouvelle déclinaison", "admin");?></a>
									<button type="submit" name="<?= (isset($id_product) && $id_product > 0) ? 'update' : 'save';?>" class="btn btn-primary btn-sm"><?=l("Sauvegarder la déclinaison", "admin");?></button>
								</span>
								</div>
						</div><!--/ .panel -->
							...
							<table class="table" id="dec-table">
								<thead>
									<tr class="nodrag nodrop">
										<th><?=l("Paire attribut - valeur", "admin");?></th>
										<th><?=l("Impact sur le prix", "admin");?></th>
										<th><?=l("Impact sur le poids", "admin");?></th>
										<th><?=l("Référence", "admin");?></th>
										<th><?=l("EAN-13", "admin");?></th>
										<th><?=l("UPC", "admin");?></th>
									</tr>
								</thead>
								<tbody>
									...
									if(!empty($product_declinaisons)) : ?>
										...
										<tr <?=($value['default_dec'] == '1') ? 'class="bg-info"' : '';?> id="<?=$value['id'];?>">
											...
											<td>
												<div class="dropdown">
													<a href="javascript:;" data-id="<?=$value['id'];?>"  class="btn btn-info edit_dec"><i class="fa fa-edit"></i> <?=l("Modifier", "admin");?></a>
													<button class="btn btn-default dropdown-toggle" data-toggle="dropdown" type="button"><span class="caret">&nbsp;</span></button>
													<ul class="dropdown-menu">
														<li><a href="javascript:;" data-id="<?=$value['id'];?>" class="delete-declinaison"><?=l("Supprimer", "admin");?></a></li>
													</ul>
												</div>
											</td>
										</tr>	
									...
									<?php else : ?>
									<tr>
										<td colspan="6"><center><i class="fa fa-warning fa-4x"></i><br> <?=l("Aucun enregistrement trouvé", "admin");?></center></td>
									</tr>
									<?php endif; ?>
								</tbody>
							</table>
						...
						<div class="alert alert-info">
							<?=l("La ligne en bleu est la déclinaison par défaut.", "admin");?><br>
							<?=l("Un produit qui possède des déclinaisons doit avoir une déclinaison par défaut.", "admin");?>
							</ul>
						</div>
					</div><!--/ .panel-body -->
					<div class="panel-footer">
						<button type="button" class="btn btn-default" onclick="window.location='?module=products';"><?=l("Terminer", "admin");?></button>
						<span class="pull-right">
							<a href="javascript:;" class="btn btn-success add_dec"><?=l("Nouvelle déclinaison", "admin");?></a>
							<button type="submit" name="<?= (isset($id_product) && $id_product > 0) ? 'update' : 'save';?>" class="btn btn-primary"><?= (isset($id_product) && $id_product > 0) ? l("Mise à jour", "admin") : l("Sauvegarder et rester", "admin");?></button>
						</span>
						</div><!--/ .panel-footer -->
				</div><!--/ .panel -->
				<?php else: ?>
				<div class="alert alert-info">
					<h4><?=l("Il y a 1 avertissement.", "admin");?></h4>
					<?=l("Vous devez enregistrer ce produit avant d'y ajouter des déclinaisons.", "admin");?>
				</div>
				<?php endif; ?>
			</div><!--/ .tab-pane -->
			...
			<?php if(isset($id_product) && $id_product > 0) : ?>
				<div class="panel panel-default">
					<div class="panel-heading"><?=l("QUANTITÉS DISPONIBLES À LA VENTE", "admin");?></div>
					<div class="panel-body">
						<div class="alert alert-info">
							<?=l("Cette interface vous permet de gérer les quantités disponibles à la vente pour ce produit. Elle vous permet également de gérer les déclinaisons produit dans la boutique en cours.", "admin");?><br>
							<?=l("Notez que si un produit du pack a plusieurs déclinaisons, c'est sa déclinaison par défaut qui sera utilisée pour les mouvements de stocks.", "admin");?>
						</div>
						...
						<div class="form-group">
						  <div class="col-lg-9 col-lg-offset-3">
						    <table class="table" id="quantities">
					      	<thead>
						        <tr>
						          <th width="100"><?=l("Quantité", "admin");?></th>
						          <th width="150"><?=l("Quantité minimale", "admin");?></th>
						          <th><?=l("Nom de produit", "admin");?></th>
						        </tr>
						      </thead>
						      <tbody>
				...
				<div class="alert alert-info">
					<h4><?=l("Il y a 1 avertissement.", "admin");?></h4>
					<?=l("Vous devez enregistrer ce produit avant d'y ajouter des quantités.", "admin");?>
				</div>
			<?php endif; ?>
			</div><!--/ .tab-pane -->
			...
			<?php if(isset($id_product) && $id_product > 0) : ?>
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div class="form-group">
							<label class="col-md-3 control-label" for="filer_input"><?=l("Ajouter une nouvelle image à ce produit", "admin");?></label>  
							<div class="col-md-6">
								<div class="input-group">
						...
						<div class="panel-subheading">
							...
							<strong><?=l("Sélectionnez l'image en vedette.", "admin");?></strong>
						</div>
						<div class="col-sm-12">
				...
				<div class="alert alert-info">
					<h4><?=l("Il y a 1 avertissement.", "admin");?></h4>
					<?=l("Vous devez enregistrer ce produit avant d'y ajouter des images.", "admin");?>
				</div>
				<?php endif; ?>
			</div><!--/ .tab-pane -->
			...
			if(isset($id_product) && $id_product > 0) : ?>
				...
				<form class="form-horizontal" id="feature_form" method="post" action="ajax/products/feature_product/form.php">
					<div class="panel-heading">
						<?=l("Attribuer des caractéristiques à ce produit", "admin");?>
					</div>
					<div class="panel-body">
						<div class="alert alert-info">
							<?=l("Vous pouvez préciser une valeur pour chaque caractéristique pertinente concernant ce produit ; les champs vides ne seront pas affichés.", "admin");?><br>
							<?=l("Vous pouvez définir une valeur spécifique, ou choisir parmi les valeurs existantes prédéfinies que vous avez ajoutées précédemment.", "admin");?>
						</div>
      				...
      				$id_lang = $_CONFIG['lang'];
							...
							<table class="table" id="features">
								<thead>
									<tr>
										<th><?=l("Caractéristique", "admin");?></span></th>
										<th><?=l("Valeur prédéfinie", "admin");?></span></th>
										<th><u><?=l("OU", "admin");?></u> <?=l("Valeur personnalisée", "admin");?></th>
										<th style="width: 60px;"><?=l("Actions", "admin");?></th>
									</tr>
								</thead>
								<tbody>
									...
									<?php foreach ($features as $key => $feature) : ?>
										<tr>
											...
											<td>
												...
												<?php else : ?>
													<?=l("N/D", "admin");?> - <a target="_blank" href="?module=feature_values&amp;action=add"><i class="fa fa-plus-square"></i> <?=l("Ajouter une valeur prédéfinie dans un premier temps", "admin");?></a> 	
												<?php endif; ?>
											</td>
			...
			<div class="tab-pane fade" id="Attachments">
				<?php if(isset($id_product) && $id_product > 0) : ?>
				<div class="panel panel-default">
					<div class="panel-heading"><?=l("Documents joints", "admin");?></div>
					<div class="panel-body">
						<div class="form-group">
							<label class="col-md-3 control-label" for="attachment_name"><?=l("Nom du fichier *", "admin");?></label>  
							<div class="col-md-6">
								<input type="text" name="attachment[name]" id="attachment_name" value="" class="form-control">
							</div>
						</div>
						<div class="form-group">
							<label class="col-md-3 control-label" for="description"><?=l("Description", "admin");?></label>  
							<div class="col-md-6">
								<textarea rowspan="5" id="description" name="attachment[description]" class="form-control"></textarea>
							</div>
						</div>
						<div class="form-group">
							<label class="col-md-3 control-label" for="attachments"><?=l("Fichier", "admin");?></label>  
							<div class="col-md-6">
						...
						<div class="form-group">
							<div class="col-sm-2 col-sm-offset-3">
								<button type="submit" name="<?= (isset($id_product) && $id_product > 0) ? 'update' : 'save';?>" class="btn btn-success"><i class="fa fa-send-o"></i> <?=l("Envoyer", "admin");?></button>
							</div>
						</div>
						<div class="panel-subheading">
							<i class="fa fa-file"></i>
							<strong><?=l("Liste des fichiers attachés", "admin");?></strong>
						</div>
						<div class="table-responsive">
							<table class="table" id="attachments">
								<thead>
									<tr class="nodrag nodrop">
										<th><?=l("Nom du fichier", "admin");?></th>
										<th><?=l("Description", "admin");?></th>
										<th width="105"><?=l("Actions", "admin");?></th>
									</tr>
								</thead>
								<tbody>
									<?php 
									if(!empty($product_attachments)) : ?>
										...
										<tr id="<?=$value['id'];?>">
											...
											<td><a href="javascript:;" id="<?=$value['id'];?>" data-attachment="<?=$value['attachment'];?>" class="btn btn-default delete_att"><?=l("Supprimer", "admin");?></a></td>
										</tr>	
										<?php endforeach; ?>
									<?php else : ?>
									<tr>
										<td colspan="6"><center><i class="fa fa-warning fa-4x"></i><br> <?=l("Aucun attachment trouvé", "admin");?></center></td>
									</tr>
									<?php endif; ?>
								</tbody>
							</table>
	...
	$('.edit_dec').on('click', function() {
		...
		$.ajax({
			...
			success:function(data){
				...
				var obj = jQuery.parseJSON(images_array);
				$.each(obj, function(key,value) {
					$("#dec_images input:checkbox[value="+ value +"]").attr("checked", true);
				});
