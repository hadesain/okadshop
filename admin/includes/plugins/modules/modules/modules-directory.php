...
<div class="top-menu padding0">
  <div class="top-menu-title">
    <h3><i class="fa fa-puzzle-piece"></i> <?=l("Liste des Modules", "admin");?></h3>
  </div>
  <!--div class="top-menu-button">
    <button type="button" class="btn btn-primary"><?php //echo _("Add a new module", "admin");?></button>
  </div-->
</div><br>
  ...
  <form id="filterForm" method="get" name="filterForm">
    ...
    <div class="col-sm-3 left0">
      <select class="form-control" id="install_filter" name="installed">
        <option value="" selected><?=l("Installés et non installés", "admin");?></option>
        <option value="1" <?=(isset($_GET['installed']) && $_GET['installed'] == "1") ? "selected" : ""; ?>><?=l("Modules installés", "admin");?></option>
        <option value="0" <?=(isset($_GET['installed']) && $_GET['installed'] == "0") ? "selected" : ""; ?>><?=l("Modules non installés", "admin");?></option>
      </select>
    </div>
    <div class="col-sm-3 left0">
      <select class="form-control" id="status_filter" name="active">
        <option value="" selected><?=l("Activés &amp; désactivés", "admin");?></option>
        <option value="1" <?=(isset($_GET['active']) && $_GET['active'] == "1") ? "selected" : ""; ?>><?=l("Modules activés", "admin");?></option>
        <option value="0" <?=(isset($_GET['active']) && $_GET['active'] == "0") ? "selected" : ""; ?>><?=l("Modules désactivés", "admin");?></option>
      </select>
    </div>
    <div class="col-sm-2 left0">
      <select class="form-control" id="author" name="author">
        <option value=""><?=l("Tous les Auteurs", "admin");?></option>
        <?php 
    ...
    <div class="col-sm-4 left0 right0">
      ...
      <input autocomplete="off" class="form-control" id="keywords" name="keywords" placeholder="<?=l("Rechercher...", "admin");?>" type="text" value="<?=isset($_GET['keywords']) ? $_GET['keywords'] : '';?>">
  ...
  <div class="panel panel-default">
    <div class="panel-body">
      <div class="col-sm-3 left0" id="categories">
        <div class="list-group">
          <?php if(!empty($categories)) : ?>
            <a onclick="extend_search('category', '');return false" class="list-group-item <?=( !isset($_GET['category']) || empty($_GET['category']) ) ? 'active' : '';?>" href="javascrip:;"><?=l("Tous les Modules", "admin");?></a>
      ...
      <div class="col-sm-9 left0 right0" id="modules">
        ...
        <div class="btn-group">
          <button class="btn btn-default dropdown-toggle" data-toggle="dropdown" type="button"><?=l("actions groupées", "admin");?><span class="caret"></span></button>
          <ul class="dropdown-menu">
            <li><a href="javascript:;" class="bulk_install"><i class="fa fa-plus-square"></i>&nbsp; <?=l("Installer la sélection", "admin");?></a></li>
            <li><a href="javascript:;" class="bulk_uninstall"><i class="fa fa-minus-square"></i>&nbsp; <?=l("Désinstaller la sélection", "admin");?></a></li>
            <li class="divider"></li>
            <li><a href="javascript:;" class="bulk_enable"><i class="fa fa-power-off"></i>&nbsp; <?=l("Activer la sélection", "admin");?></a></li>
            <li><a href="javascript:;" class="bulk_disable"><i class="fa fa-power-off"></i>&nbsp; <?=l("Disactiver la sélection", "admin");?></a></li>
            <li class="divider"></li>
            <li><a href="javascript:;" class="bulk_delete"><i class="fa fa-trash"></i>&nbsp; <?=l("Supprimer la sélection", "admin");?></a></li>
          </ul>
        </div>
        <table class="table table-striped table-bordered">
                      ...
                      <?php if( $module['installed'] == 1 ) : ?>
                        <?php if( $module['active'] == 1 ) : ?>
                          <?php if( isset($module['config']) && $module['config'] != "" ) : ?>
                            <a class="btn btn-info config" href="<?php echo '?module=modules&slug='. $key .'&page='. $module['config']; ?>"><i class="fa fa-cogs"></i> <?=l("Configuration", "admin");?></a>
                          <?php else : ?>
                            <a class="btn btn-default disable" href="javascript:;"><i class="fa fa-power-off"></i> <?=l("Désactiver", "admin");?></a>
                          <?php endif; ?>
                        <?php elseif( $module['active'] == 0 ) : ?>
                          <a class="btn btn-default enable" href="javascript:;"><i class="fa fa-power-off"></i> <?=l("Activer", "admin");?></a>
                        <?php endif; ?>
                      <?php elseif( $module['installed'] == 0 ) : ?>
                        <a class="btn btn-success install" href="javascript:;"><i class="fa fa-cog"></i> <?=l("Installer", "admin");?></a>
                      <?php endif; ?>
                      <!-- Buttons -->
                      <button class="btn btn-default dropdown-toggle" data-toggle="dropdown" type="button"><span class="caret">&nbsp;</span></button>
                      <ul class="dropdown-menu">
                        <?php if( isset($module['config']) && $module['config'] != "" ) : ?>
                          <li><a class="disable" href="javascript:;"><i class="fa fa-power-off"></i> <?=l("Désactiver", "admin");?></a></li>
                          <li class="divider"></li>
                        <?php endif; ?>
                        <?php if( $module['installed'] == 1 ) : ?>
                          <li><a href="javascript:;" class="uninstall"><i class="fa fa-minus-square"></i> <?=l("Désinstaller", "admin");?></a></li>
                          <li class="divider"></li>
                        <?php endif; ?>
                        <!--li><a href="javascript:;" class="favorite"><i class="fa fa-star"></i> Ajouter aux favoris</a></li-->
                        <li><a href="javascript:;" class="delete"><i class="fa fa-trash"></i> <?=l("Supprimer", "admin");?></a></li>
                      </ul>
                    </div>
                  </div>
                </td>
          ...
          <?php else : ?>
            <tr>
              <td colspan="3">
                <center><strong><?=l("No Module found.", "admin");?></strong></center>
              </td>
            </tr>
          <?php endif; ?>
          </tbody>
...
$('.actions').on('click', '.delete', function(){
  var choice = confirm('<?=l("Cette action supprime définitivement le module sur votre serveur. Êtes-vous vraiment sûr ?", "admin");?>');
