  ...
  <div class="top-menu padding0">
    <div class="top-menu-title">
      <h3>
      	...
      	<?= (isset($id_zone) && $id_zone > 0) ? l("Modifier une Zone") : l("Ajouter une Zone", "admin");?>
      </h3>
    </div>
    <div class="top-menu-button">
      <button type="submit" name="<?= (isset($id_zone) && $id_zone > 0) ? 'update' : 'save';?>" class="btn btn-primary pull-right"><?= (isset($id_zone) && $id_zone > 0) ? l("Mise à jour") : l("Enregister", "admin");?></button>
    </div>
  </div><br>

  <div class="panel panel-default">
		<div class="panel-heading"><?=l("Zones", "admin");?></div>
		<div class="panel-body">

			<div class="form-group">
			  <label class="control-label col-lg-3 required">
			  	<span class="label-tooltip" data-html="true" data-original-title="<?=l("Nom de la zone (ex. : Afrique, Europe, Pays voisins).", "admin");?>" data-toggle="tooltip" title=""><?=l("Nom *", "admin");?></span>
			  </label>
			  <div class="col-lg-3">
        ...
        <div class="col-sm-3">
          <input type="checkbox" name="zone[active]" class="switch" id="handling" data-on-text="<?=l("OUI", "admin");?>" data-off-text="<?=l("NON", "admin");?>" value="1" <?= ($zone[0]['active']=="1") ? 'checked' : '';?>/>
        </div>
      </div>

		</div><!--/ .panel-body -->
		<div class="panel-footer">
			<button type="button" class="btn btn-default" onclick="window.location='?module=zones';"><?=l("Fermer", "admin");?></button>
			<button type="submit" name="<?= (isset($id_zone) && $id_zone > 0) ? 'update' : 'save';?>" class="btn btn-primary pull-right"><?= (isset($id_zone) && $id_zone > 0) ? l("Mise à jour") : l("Enregister", "admin");?></button>
		</div><!--/ .panel-footer -->
	</div><!--/ .panel -->
