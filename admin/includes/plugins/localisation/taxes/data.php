  ...
  <div class="top-menu padding0">
    <div class="top-menu-title">
      <h3>
      	...
      	<?= (isset($id_tax) && $id_tax > 0) ? l("Modifier une Taxe") : l("Ajouter une Taxe", "admin");?>
      </h3>
    </div>
    <div class="top-menu-button">
      <button type="submit" name="<?= (isset($id_tax) && $id_tax > 0) ? 'update' : 'save';?>" class="btn btn-primary pull-right"><?= (isset($id_tax) && $id_tax > 0) ? l("Mise à jour") : l("Enregister", "admin");?></button>
    </div>
  </div><br>

  <div class="panel panel-default">
		<div class="panel-heading"><?=l("Taxes", "admin");?></div>
		<div class="panel-body">
			...
			<div class="form-group">
				<label class="control-label col-lg-3 required">
					<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="<?=l("Format : XX.XX ou XX.XXX (ex. : 19.60 ou 13.925) - Caractères interdits <>;=#{}", "admin");?>"><?=l("Taux *", "admin");?></span>
				</label>
			  <div class="col-lg-3">
        ...
        <div class="col-sm-3">
          <input type="checkbox" name="tax[active]" class="switch" id="handling" data-on-text="<?=l("OUI", "admin");?>" data-off-text="<?=l("NON", "admin");?>" value="1" <?= ($tax[0]['active']=="1") ? 'checked' : '';?>/>
        </div>
      </div>

		</div><!--/ .panel-body -->
		<div class="panel-footer">
			<button type="button" class="btn btn-default" onclick="window.location='?module=taxes';"><?=l("Fermer", "admin");?></button>
			<button type="submit" name="<?= (isset($id_tax) && $id_tax > 0) ? 'update' : 'save';?>" class="btn btn-primary pull-right"><?= (isset($id_tax) && $id_tax > 0) ? l("Mise à jour") : l("Enregister", "admin");?></button>
		</div><!--/ .panel-footer -->
	</div><!--/ .panel -->
