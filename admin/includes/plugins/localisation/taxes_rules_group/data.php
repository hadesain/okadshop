...
<form class="form-horizontal" method="post" action="">
  ...
  <div class="top-menu padding0">
    <div class="top-menu-title">
      <h3>
      	...
      	<?= (isset($id_group) && $id_group > 0) ? l("Modifier une règles de taxes") : l("Ajouter une règles de taxes", "admin");?>
      </h3>
    </div>
    <div class="top-menu-button">
      <button type="submit" name="<?= (isset($id_group) && $id_group > 0) ? 'update' : 'save';?>" class="btn btn-primary pull-right"><?=l("Sauvegarder et rester", "admin");?></button>  
    </div>
  </div><br>
  ...
  <div class="panel panel-default">
		<div class="panel-heading"><?=l("Règles de taxes", "admin");?></div>
		<div class="panel-body">

			<div class="form-group">
			  <label class="control-label col-lg-3 required">
			  	<span class="label-tooltip" data-html="true" data-original-title="<?=l("Nom de la taxe à afficher dans le panier et sur la facture (ex. : &quot;TVA&quot;). - Caractères interdits &lt;&gt;;=#{}", "admin");?>" data-toggle="tooltip" title=""><?=l("Nom *", "admin");?></span>
			  </label>
			  <div class="col-lg-3">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-3">
          <input type="checkbox" name="group[active]" class="switch" id="handling" data-on-text="<?=l("OUI", "admin");?>" data-off-text="<?=l("NON", "admin");?>" value="1" <?= ($group[0]['active']=="1") ? 'checked' : '';?>/>
        </div>
      </div>


      <div class="panel-subheading">
        <strong class="edit_tax"><?=l("NOUVELLE RÈGLE DE TAXE", "admin");?></strong>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="col-lg-2">
          <select name="tax_rule[id_country]" class="form-control" id="countries">
            <option value="0" selected><?=l("Toutes", "admin");?></option>
      ...
      <div class="form-group">
        <label class="col-md-3 control-label"><?=l("Taxes", "admin");?></label> 
        <div class="col-sm-2">
      ...
      <div class="form-group">
        <label class="control-label col-lg-3"><?=l("Description", "admin");?></label>
        <div class="col-lg-5">
          <input type="text" name="tax_rule[description]" id="description" value="" class="form-control">
        </div>
      </div>
      <div class="panel-subheading">
        <strong><?=l("RÈGLE DE TAXE", "admin");?></strong>
      </div>
      <div class="col-sm-10 col-sm-offset-1">
        <table class="table bg-white">
          <thead>
            <tr>
              <th width="120"><?=l("Pays", "admin");?></th>
              <th width="120"><?=l("Taxes", "admin");?></th>
            ...
            <?php if( !empty($taxes_rules) ) : ?>
              <?php foreach ($taxes_rules as $key => $rule) : ?>
                <tr id="<?php echo $rule['id']; ?>">
                  ...
                  <td>
                    <a href="javascript:;" class="btn btn-default edit_tax" title="<?=l("Modifier cette règle", "admin");?>"><i class="fa fa-edit"></i></a>
                    <a href="?module=taxes_rules_group&action=edit&id=<?php echo $id_group; ?>&delete=<?php echo $rule['id']; ?>" class="btn btn-danger delete_tax" title="<?=l("Supprimer cette règle", "admin");?>"><i class="fa fa-trash"></i></a>
                  </td>
                </tr>
              <?php endforeach; ?>
            <?php endif; ?>
		...
		<div class="panel-footer">
			<button type="button" class="btn btn-default" onclick="window.location='?module=taxes_rules_group';"><?=l("Fermer", "admin");?></button>
		  <button type="submit" name="<?= (isset($id_group) && $id_group > 0) ? 'update' : 'save';?>" class="btn btn-primary pull-right"><?=l("Sauvegarder et rester", "admin");?></button>
    </div><!--/ .panel-footer -->
	</div><!--/ .panel -->
</form>
...
$(document).ready(function(){
  ...
  $('.edit_tax').on('click', function(){
    ...
    $('.edit_tax').val('<?=l("EDITION DE LA RÈGLE DE TAXE", "admin");?>');
    //message de notif
    $.bootstrapGrowl("<?=l("Vous êtes en mode de modification de règle de taxe.", "admin");?>" , {
      type: 'info',
      align: 'right',
    });
  });
