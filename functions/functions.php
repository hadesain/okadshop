...
function goConnected(){
	if ( isset($_SESSION['user']) ) {
		...
		echo '<script>window.location.href = "'.HOME_URL.'account/login"</script>';
	}
}
    ...
    if ($category && !empty($category)) {
      foreach ($category as $key => $value) {
        ...
        if ($sub_cat && !empty($sub_cat)) {
          echo '<li><a href="'.HOME_URL.'category/'.$value['id'].'-'.$value['permalink'].'">'.$value['name'].'</a>  <span class="grower CLOSE" data-toggle="collapse" data-target="#cat_tab_'.$value['id'].'"></span>';
          echo '<ul id="cat_tab_'.$value['id'].'" class="collapse"> ';
          getCatList($value['id']);
          echo '</ul></li>';
        }else{
          echo '<li><a href="'.HOME_URL.'category/'.$value['id'].'-'.$value['permalink'].'">'.$value['name'].'</a></li>';
        }
      }
    }
  }
        ...
        if ($sub_cat && !empty($sub_cat) && $more) {
           /* $span_class = "grower CLOSE";
            if (!$first) {
          	...
          	$output.= '<li><a href="'.HOME_URL.'category/'.$value['id'].'-'.$value['permalink'].'">'.$value['name'].'</a>  <span class="grower CLOSE" data-toggle="collapse" data-target="#cat_tab_'.$value['id'].'"></span>';
          	$output.= '<ul id="cat_tab_'.$value['id'].'" class="collapse"> ';
            $output.= getPanelCatList($value['id'],true);
          	$output.= '</ul></li>';*/

            if ($first) {
              $output.= '<li><a href="'.HOME_URL.'category/'.$value['id'].'-'.$value['permalink'].'">'.$value['name'].'</a>  <span class=" OPEN" data-toggle="collapse" data-target="#cat_tab_'.$value['id'].'"></span>';
              ...
              $output.= '</ul></li>';
           	...
           	$output.= '<li><a href="'.HOME_URL.'category/'.$value['id'].'-'.$value['permalink'].'">'.$value['name'].'</a></li>';
        }
      }
    }
    return 	$output;
   ...
   function getSitemapCatList($id){
    ...
    if ($category && !empty($category)) {
      foreach ($category as $key => $value) {
        $sub_cat = getcategoryByParent($value['id']);
        if ($sub_cat && !empty($sub_cat)) {
          	$output.= '<li><a href="'.HOME_URL.'category/'.$value['id'].'-'.$value['permalink'].'">'.$value['name'].'</a>';
          	$output.= '<ul> ';
